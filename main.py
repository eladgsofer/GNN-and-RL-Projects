...
def vectorised_BFS(A, start_ind):
    ...
    while pathing_improved:
        ...
        if ((current_routes[np.arange(len(A)) != start_ind] - new_routes[
            np.arange(len(A)) != start_ind]) ** 2).sum() < 10 ** (-8):
...
def show_graph_with_labels(adjacency_matrix):
    ...
    weighted_edges = [(rows.tolist()[i], cols.tolist()[i], round(adjacency_matrix[rows, cols].tolist()[i], 2)) for i in
                      range(len(rows.tolist()))]
